...
/*   Updated: 2023/11/30 20:42:09 by yunjcho          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
bool ScalarConverter::checkArgsPseudo(std::string &argv) {
  std::string float_pseudos[6] = {"-inff", "+inff", "inff",
                                  "-nanf", "+nanf", "nanf"};
  std::string double_pseudos[6] = {"-inf", "+inf", "inf",
                                   "-nan", "+nan", "nan"};
...
argvType ScalarConverter::checkArgsType(std::string &argv) {
  const std::string NUMBERS = "0123456789";

  if (argv.length() == 1 && argv.find_first_of(NUMBERS) == std::string::npos) {
    if (argv.c_str()[0] >= 0 && argv.c_str()[0] <= 127) return (CHAR_TYPE);
  } else {
    if (argv.find_first_of(NUMBERS) != std::string::npos) {
      if (argv.find(".") != std::string::npos) {
        ...
        if (argv.find_last_of("f") == (argv.length() - 1)) {
          ...
          if (tmp.find("f") != std::string::npos) return (ETC_TYPE);
          return (FLOAT_TYPE);
        }
        return (DOUBLE_TYPE);
      }
...
std::string ScalarConverter::createFloatStr(float floatVal, double checkVal,
                                            std::string &argv) {
  std::stringstream floatStream;
  float transFloat = 0.0;
  std::string doubleStr = std::to_string(checkVal);

  transFloat = std::strtof(argv.c_str(), NULL);
  floatStream << std::fixed << std::setprecision(1) << floatVal;
  std::string convertStr = floatStream.str();
  if (transFloat != transFloat || checkArgsPseudo(_convertFloat) ||
      checkArgsPseudo(doubleStr)) {
    _convertFloat = std::to_string(transFloat) + "f";  // nan
...
std::string ScalarConverter::createDoubleStr(double doubleVal, double checkVal,
                                             std::string &argv) {
  ...
  if (checkVal != checkVal || checkArgsPseudo(_convertFloat) ||
      checkArgsPseudo(doubleStr)) {
    _convertFloat = std::to_string(transFloat) + "f";  // nan
  } else if (checkVal == 0.0)
    _convertDouble = "0.0";
...
void ScalarConverter::convertChar(std::string &argv) {
  ...
  charStream << std::fixed << std::setprecision(1)
...
void ScalarConverter::convertDouble(std::string &argv) {
  ...
  if (!_convertDouble.compare("impossible")) convertInvalid();
}
...
void ScalarConverter::printValues(void) {
  if (_convertChar.compare("impossible") &&
      _convertChar.compare("Non displayable"))
